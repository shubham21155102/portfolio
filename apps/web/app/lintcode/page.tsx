    ...
    const fetchQuestions = async () => {
      ...
      try {
        const res = await fetch(
          `https://api.shubhamiitbhu.in/questions/lintcode`,
          {
            method: "POST",
            headers: {
              ...
              questions.map((question: any) => (
                <tr key={question.problem_id}>
                  ...
                  <td className="px-4 py-2">
                    <Link
                      href={`https://www.lintcode.com/problem/${question.problem_id}/`}
                      passHref
                  ...
                  <td className="px-4 py-2">
                    {question.problem_tags.map((e: any) => {
                      return (
                        <span
                          key={e}
                          className="px-2 py-1 mr-2 text-sm font-bold text-white bg-gray-800 rounded-md"
                        >
                          {e.unique_name}
                        </span>
                      );
